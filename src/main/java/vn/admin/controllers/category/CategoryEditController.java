...
public class CategoryEditController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		Category cate = cateService.findCategoryByID(Integer.parseInt(id));
		req.setAttribute("cate", cate);
		List<Category> cateList = cateService.findAllCategory();
		req.setAttribute("cateList", cateList);
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/admin/category/category_edit.jsp");
		...
		try {
			...
			for (FileItem item : items) {
				if (item.getFieldName().equals("_name")) {
					...
					cate.set_id(Integer.parseInt(item.getString()));
				} else if (item.getFieldName().equals("isDeleted")) {
					cate.setDeleted(Boolean.parseBoolean(item.getString()));
				}
			}
